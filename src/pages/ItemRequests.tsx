...
import { useItemRequests, useItems, useUsers, useCreateItemRequest, useUpdateItemRequest, useSubmitItemRequest, useDeleteItemRequest } from '@/hooks/useApi';
...
const ItemRequests: React.FC = () => {
  ...
  const [editingRequest, setEditingRequest] = useState<ItemRequestType | null>(null);
  const [viewingRequest, setViewingRequest] = useState<ItemRequestType | null>(null);
  ...
  const filteredRequests = requests
    ...
    .filter((r) =>
      r.transactionNumber.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const onSubmit = async () => {
    ...
    try {
      if (editingRequest) {
        await updateRequest.mutateAsync({ id: editingRequest.id, updates: formData });
        toast.success('Request updated successfully');
      } else {
        await createRequest.mutateAsync({ ...formData, createdBy: user?.id || '' });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {isDepartmentUser ? 'Submit requests for items you need' : 'Manage item requests from departments'}
          </p>
        </div>
        {(isDepartmentUser || user?.role === 'admin' || user?.role === 'warehouse_staff') && (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredRequests.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="font-medium">{r.transactionNumber}</TableCell>
                      <TableCell>{r.department}</TableCell>
                      <TableCell>{r.requestedBy}</TableCell>
                      <TableCell>{format(new Date(r.requiredDate), 'dd/MM/yyyy')}</TableCell>
                      <TableCell><StatusBadge status={r.status} /></TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openViewDialog(r)}>
                          ...
                          {r.status === 'DRAFT' && r.createdBy === user?.id && (
                            <>
                              ...
                              <Button variant="ghost" size="icon" onClick={() => setSubmitId(r.id)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingRequest ? 'Edit Request' : 'New Item Request'}</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  readOnly={isDepartmentUser && !!user?.department}
                />
              </div>
              <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={onSubmit}>{editingRequest ? 'Update' : 'Create'} as Draft</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{viewingRequest.transactionNumber}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground">Status</Label>
                  <div className="mt-1"><StatusBadge status={viewingRequest.status} /></div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{format(new Date(viewingRequest.requiredDate), 'dd/MM/yyyy')}</p>
                </div>
              </div>
              ...
              {viewingRequest.status === 'APPROVED' && viewingRequest.signatureImage && (
                <div>
                  <Label className="text-muted-foreground">Approval Signature</Label>
                  <div className="border rounded-lg p-2 mt-2 bg-card">
                    <img src={viewingRequest.signatureImage} alt="Signature" className="max-w-full h-auto" />
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
                    Approved by {getUserName(viewingRequest.approvedBy || '')} on{' '}
                    {viewingRequest.approvedAt && format(new Date(viewingRequest.approvedAt), 'dd/MM/yyyy HH:mm')}
                  </p>
                </div>
              )}
              ...
              {viewingRequest.status === 'REJECTED' && (
                <div>
                  ...
                  <p className="text-destructive">{viewingRequest.rejectionReason}</p>
                </div>
              )}
            </div>
          )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to submit this request for approval? You won't be able to edit it after submission.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
