...
const PurchaseOrders: React.FC = () => {
  ...
  const filteredOrders = orders.filter((o) =>
    o.transactionNumber.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const onSubmit = async () => {
    ...
    try {
      if (editingOrder) {
        ...
        await createOrder.mutateAsync({ ...formData, createdBy: user?.id || '' });
              ...
              <Table>
                ...
                <TableBody>
                  {filteredOrders.map((o) => (
                    <TableRow key={o.id}>
                      <TableCell className="font-medium">{o.transactionNumber}</TableCell>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{viewingOrder.transactionNumber}</p>
                </div>
                <div>
              ...
              {viewingOrder.status === 'REJECTED' && (
                <div>
                  ...
                  <p className="text-destructive">{viewingOrder.rejectionReason}</p>
                </div>
              )}
            </div>
          )}
