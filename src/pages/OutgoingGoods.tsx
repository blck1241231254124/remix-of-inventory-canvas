...
import { useOutgoingGoods, useItems, useUsers, useCreateOutgoingGoods, useUpdateOutgoingGoods, useSubmitOutgoingGoods, useDeleteOutgoingGoods } from '@/hooks/useApi';
import { OutgoingGoods as OutgoingGoodsType, TransactionItem } from '@/types';
...
import { Plus, Pencil, Trash2, Search, Send, Eye, X, AlertTriangle } from 'lucide-react';
...
const OutgoingGoods: React.FC = () => {
  ...
  const [editingTransaction, setEditingTransaction] = useState<OutgoingGoodsType | null>(null);
  const [viewingTransaction, setViewingTransaction] = useState<OutgoingGoodsType | null>(null);
  ...
  const getItemStock = (itemId: string) => {
    return items.find((i) => i.id === itemId)?.stock || 0;
  };
  ...
  const resetForm = () => {
    setFormData({ destination: '', requestedBy: '', notes: '', items: [] });
  ...
  const onSubmit = async () => {
    if (!formData.destination || !formData.requestedBy || formData.items.length === 0) {
    ...
    try {
      if (editingTransaction) {
        await updateTransaction.mutateAsync({ id: editingTransaction.id, updates: formData });
        toast.success('Transaction updated successfully');
      } else {
        await createTransaction.mutateAsync({ ...formData, createdBy: user?.id || '' });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage outgoing goods transactions</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingTransaction ? 'Edit Transaction' : 'New Outgoing Goods'}</DialogTitle>
          </DialogHeader>
                      ...
                      {formData.items.map((item) => {
                        ...
                        return (
                          <TableRow key={item.itemId}>
                            ...
                            <TableCell className={isOverStock ? 'text-destructive font-medium' : ''}>
                              {item.quantity}
                              {isOverStock && <AlertTriangle className="h-3 w-3 inline ml-1" />}
                            </TableCell>
          ...
          <DialogFooter>
            ...
            <Button onClick={onSubmit}>{editingTransaction ? 'Update' : 'Create'} as Draft</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1"><StatusBadge status={viewingTransaction.status} /></div>
                </div>
                <div>
                  <Label className="text-muted-foreground">Destination</Label>
                  <p className="font-medium">{viewingTransaction.destination}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground">Requested By</Label>
                  <p className="font-medium">{viewingTransaction.requestedBy}</p>
                </div>
              </div>
              ...
              {viewingTransaction.status === 'APPROVED' && viewingTransaction.signatureImage && (
                <div>
                  ...
                  <div className="border rounded-lg p-2 mt-2 bg-card">
                    <img src={viewingTransaction.signatureImage} alt="Signature" className="max-w-full h-auto" />
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
                    Approved by {getUserName(viewingTransaction.approvedBy || '')} on{' '}
                    {viewingTransaction.approvedAt && format(new Date(viewingTransaction.approvedAt), 'dd/MM/yyyy HH:mm')}
                  </p>
                </div>
              )}
